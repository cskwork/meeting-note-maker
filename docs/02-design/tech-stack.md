...
| STT | **@huggingface/transformers v3** (구 `@xenova/transformers`) | Moonshine Korean ONNX를 브라우저 WASM CPU에서 실행, 단일 npm 패키지로 워커 통합 |
| STT model | `onnx-community/moonshine-tiny-ko-ONNX` only | 한국어 전용·소형·로컬 실행. v1.0.0 UI에서는 다른 STT 모델을 노출하지 않음 |
...
현재 UI 선택지:
- `moonshine-tiny-ko-ONNX`: Korean ASR 전용, 27M parameters, WASM CPU

Whisper 계열은 과거 비교 후보로만 남기고 v1.0.0 UI에는 노출하지 않는다.
